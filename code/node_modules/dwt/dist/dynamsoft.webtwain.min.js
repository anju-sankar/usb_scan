...
(function(h) {
    ...
    h.mix(d.prototype, {
        ...
        _convertByteSize: function(j) {
            ...
            return Math.max(j, .1).toFixed(1) + ["KB", "MB", "GB", "TB"][k]
        },
        _init: function() {
            var j = this;
            var m = {
            ...
            for (F = 0; F < q; F++) {
                for (G = 0; G < A; G++) {
                    ...
                    for (var B = Math.floor(F * E); B < (F + 1) * E; B++) {
                        var s = Math.abs(g - (B + .5)) / C,
                            k = (G + .5) * m,
                            u = s * s;
                        for (var e = Math.floor(G * m); e < (G + 1) * m; e++) {
                            var t = Math.abs(k - (e + .5)) / z,
                                r = Math.sqrt(u + t * t);
                            if (r >= -1 && r <= 1) {
                                v = 2 * r * r * r - 3 * r * r + 1;
                                if (v > 0) {
...
Dynamsoft.WebTwainEnv.ShowDialog = function(f, j, k, a, g) {
    ...
    if (!b.get("J_waiting")) {
        ...
        c.style.width = "100%";
        if (a) {
            n = ['<div id="J_waiting" class="ds-dialog" style="width:455px;">', '<div class="ks-dialog-header"></div>', '<div class="ks-dialog-body">', '<div class="ks-dialog-content"><div id="', e._divInstallBody, '"></div></div></div>', '<div class="ks-dialog-footer"></div>', "</div>"]
        } else {
    ...
    if (a) {
        b.get(e._divInstallBody).innerHTML = ['<div class="ds-dialog-body" style="height:' + j + "px;width:" + f + 'px">', k, "</div>"].join("")
    } else {
        var l = g ? e._strDynamsoftWithoutClose : e._strDynamsoftWithClose;
        b.get(e._divInstallBody).innerHTML = ['<div class="ds-dialog-body" style="height:' + j + 'px">', l, k, "</div>"].join("")
    }
...
function DCP_DWT_OnClickCloseInstall(a) {
    ...
    a.detect.OnWebTwainPreExecute = function() {
        ...
        var d = [Dynamsoft.WebTwainEnv.ResourcesPath, "/reference/loading.gif"].join(""),
...
(function(c) {
                ...
                if (f <= p || n <= j) {
                    if (f <= p && n <= j) {
                        var g = f / p,
                            l = n / j;
                        if (g < l) {
                            m.w = Math.floor(p * g + .5);
                            m.h = Math.floor(j * g + .5);
                            m.zoom = g
                        } else {
                            m.w = Math.floor(p * l + .5);
                            m.h = Math.floor(j * l + .5);
                            m.zoom = l
                        }
                    } else {
                        if (f <= p) {
                            m.zoom = f / p;
                            m.w = Math.floor(f + .5);
                            m.h = Math.floor(j * m.zoom + .5)
                        } else {
                            if (n <= j) {
                                m.zoom = n / j;
                                m.w = Math.floor(p * m.zoom + .5);
                                m.h = Math.floor(n + .5)
                            }
                        }
                    }
    ...
    a.prototype.ImageMoveLeft = function() {
        var h = this,
            ...
            g;
        if (f && h.scrollH) {
            g = -.2 * h.scrollH.GetPagePositionSize();
    ...
    a.prototype.ImageMoveUp = function() {
        var h = this,
            ...
            g;
        if (f && h.scrollV) {
            g = -.2 * h.scrollV.GetPagePositionSize();
    ...
    a.prototype.__getOrigImagePos = function() {
        var k = this,
            ...
            g = k.drawArea.height;
        if (f <= k.canvasWidth) {
            h = h + (k.canvasWidth - f) * .5
        }
        if (g <= k.canvasHeight) {
            j = j + (k.canvasHeight - g) * .5
        }
        return {
            x: h,
            y: j
    ...
    a.prototype.Show = function(k) {
        ...
        if (!j.objImage || j.objImage.src == "data:," || j.objImage.width == 0 || j.objImage.height == 0 || j.objImage.src == "") {
            ...
            if (j._width > 53) {
                ...
                f.textAlign = "center";
                f.strokeText("Loading ...", f.canvas.width * .5, f.canvas.height * .5)
            }
            return false
        }
    ...
    a.prototype.__getScrollInfoWithNewZoom = function(E) {
        ...
        if (!E.bMoveCenter) {
            if (p) {
                h = v = A.objImage.width * .5;
                g = u = A.objImage.height * .5;
                n = v * r;
                l = u * r
            } else {
                h = h + y * .5;
        ...
        s = A.canvasHeight;
        if (E.bMoveCenter) {
            var C = (A.drawArea.width - f) * .5,
                m = E.x,
                k = E.newRealCenterX * r,
                w = E.newRealCenterX * x,
                F = C - k - m + w;
            G.x = C - F;
            var t = (A.drawArea.height - s) * .5,
                B = E.y,
    ...
    a.prototype.__recaculateDrawArea = function() {
        ...
        if (g) {
            ...
            if (l.drawArea.width < l.canvasWidth) {
                l.drawArea.x = (l.drawArea.width - l.canvasWidth) * .5
            } else {
                l.drawArea.x = 0
            }
            if (l.drawArea.height < l.canvasHeight) {
                l.drawArea.y = (l.drawArea.height - l.canvasHeight) * .5
            } else {
                l.drawArea.y = 0
            }
...
(function(c) {
    ...
    var f = "eng",
        ...
        e = {
            ...
            init: function(k, h) {
                var l = k,
                    j = h || {},
                    g = Math.floor(Math.random() * 1e5 + 1);
    ...
    a.prototype.updateEditor = function() {
        ...
        if (p.toolbar.self) {
            ...
            var g = Math.floor(Math.random() * 1e3 + 1).toString();
...
Dynamsoft.WebTwainEnv.ProductKey = "312D5DEDAF9D979261206D5632EB236B17F2EDA1C98E858428A7DAA36AD24B7291800218DED122F932681BA9631FEA1F91800218DED122F947F0F5E48FDAEB3317F2EDA1C98E85848FC3BC133CF49D1340000000;t0068WQAAADNMbtGYGhHLUFdRwlFsSzPolppEWoXSwtN+LqBQVtx9GZndA2wmDM1unPbz4Ww0/NrJ3Vn+60gYNgXewolIdLg=";
Dynamsoft.WebTwainEnv.Trial = true;
Dynamsoft.WebTwainEnv.ActiveXInstallWithCAB = false;
Dynamsoft.WebTwainEnv.ResourcesPath = "D:/Anju/document-scanning/node_modules/dwt/dist/";
